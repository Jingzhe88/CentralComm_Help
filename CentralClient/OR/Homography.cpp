...
int adjustRobotCameraAngle(Mat &cap_image, vector<Point2f> &test_pts)
{
	double avg_x=0;
...
SIFT sift(1, 3, 0.1, 10, 1.0);
void checkHomography(Mat &cap_image, vector<KeyPoint> &cap_kpts, Mat &cap_desc, QueryResults &ret, char GorR, int &robotCameraAngle)
{
...
//--------------------modify keyponits extractor into Brisk by ZJZ---------------------------
	...
	sift(cap_image, noArray(), cap_kpts, cap_desc);
...
#pragma omp parallel for
	...
	double timerPerImg = (double)cvGetTickCount();
		...
		ObjectRecognition::loadImage(ret_src, id, GorR, 1);
		...
		if(goodmatches.size()>10)
		{
			...
			Scalar clrset[5] = {        
				...
				if( score >0.10)
				{
					...
					robotCameraAngle = adjustRobotCameraAngle(cap_image, test_pts);
					cout << endl <<endl <<"Angle need to adjust = " << robotCameraAngle << endl<<endl;
					//getchar();
				}
