...
public:
  ...
  ObjectRecognition(Mat _mat, int _ret_num, char *dbname);
	~ObjectRecognition(){};
  //int find();
	int find(Mat &_mat, char GorR);
  int find(Mat &_mat, char GorR, int &robotCameraAngle);
static void loadImage(stringstream &ret_src, int &result_id, char RorG, bool _display=0)
{
	if (RorG == 'G' || RorG == 'g')
		ret_src <<  "G20111105/image_"; 
	else if (RorG == 'R' || RorG == 'r')
		ret_src <<  "R20111105/image_"; 
